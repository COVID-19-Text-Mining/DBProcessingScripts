...
class ChemrxivDocument(VespaDocument):
    meta = {"collection": "chemrxiv_parsed_vespa",
            "indexes": indexes
    }

    latest_version = latest_version
    unparsed_document = ReferenceField('UnparsedChemrxivDocument', required=True)

class ChemrxivParser(Parser):
    ...
    def _parse_body_text(self, doc):
        ...
        if self.parse_full_text:
            paper_fs = gridfs.GridFS(self.db, collection='Scraper_chemrxiv_org_fs')
...
class UnparsedChemrxivDocument(DynamicDocument):
    meta = {"collection": "Scraper_chemrxiv_org"
    }

    parser = ChemrxivParser()

    parsed_class = ChemrxivDocument

    parsed_document = ReferenceField(ChemrxivDocument, required=False)
    ...
    def parse(self):
        ...
        return ChemrxivDocument(**parsed_document)