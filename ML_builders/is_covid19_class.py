...
entries = list(db.entries.find({"is_covid19_ml_bool": {"$exists": False}}, projection = ["abstract", 'title', "keywords", "keywords_ML", 'category_human', 'is_covid19', 'body_text']))
...
# print(covid_count)
    db.entries.update_one({"_id": entry["_id"]}, {"$set": {"is_covid19_ml": entry["is_covid19_ml"], "is_covid19_ml_bool": entry["is_covid19_ml_bool"], "last_updated": datetime.datetime.now()}})
